...
import io.github.waterpeak.sorry.AnimatedGifEncoder
import io.github.waterpeak.sorry.getExternalStoragePublicDirectoryPicture
...
class GifEditViewModel : ViewModel() {
    ...
    private fun create(context: Context,
        ...
        val myDir = File(
            getExternalStoragePublicDirectoryPicture(
                context
            ), "sorrygif")
        myDir.mkdirs()
        ...
        if (gifFile.exists()) {
            live.value =
                GifProgress("File ${gifFile.name} exists, use another one.")
            return live
        }
...
//                ))
                ...
                live.postValue(
                    GifProgress(
                        ...
                        true,
                        true,
                        "Success.File path:${gifFile.path}",
                        gif
                    )
                )
            } catch (e: Exception) {
                ...
                while (!outputDone) {
                    ...
                    if (!outputDone) {
                        ...
                        if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                            ...
                            if (VERBOSE) Log.d(
                                TAG, "decoder output buffers changed")
                        } else if (decoderStatus == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                            val newFormat = decoder.outputFormat
                            if (VERBOSE) Log.d(
                                TAG, "decoder output format changed: " + newFormat);
                            ...
                            if (doRender) {
                                if (VERBOSE) Log.d(
                                    TAG, "awaiting decode of frame " + decodeCount);
